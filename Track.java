...
class Track {
    ...
    private String formattedDuration(int totalSeconds) {
       ...
       String formDur = "";
       int minutes = totalSeconds / 60;           //calculate minutes from the seconds given
       int seconds = totalSeconds % 60;           //the % gives the remaining seconds after calculating the minutes.
       if (seconds < 10){                     
        formDur = minutes + ":" + "0" + seconds;      //keeps the two digit format like asked
         } 
       else{
        formDur = minutes + ":" + seconds;
         }
    return formDur;
    }
}